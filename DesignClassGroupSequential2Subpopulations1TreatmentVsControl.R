# This class of group sequential designs is based on the file
...
design.evaluate.GroupSequential.OneTreatmentArm <- function(test.statistics,
                            efficacy.boundary,
                            futility.boundary,
                            alpha.allocation,
                            restrict.enrollment = FALSE,
                            boundary.to.enroll = NULL){
  stopifnot(restrict.enrollment == FALSE) # Since enrollment cannot be restricted for standard group sequential designs.
  if(restrict.enrollment == FALSE){
    # Number of stages
    K = length(alpha.allocation)/2
    ...
    for(i in 1:n.sim){
      ...
      for(k in 1:K){
        ...
        if(test.statistics[i, 2* (k-1) + 1] > est.eff.bound[2* (k-1) + 1]){
          reject.hyp[i, 1] = 1
        }
        # Do the testing in subpopulation 2
        if(test.statistics[i, 2* (k-1) + 2] > est.eff.bound[2* (k-1) + 2]){
          reject.hyp[i, 2] = 1
        }
        # Decide early stopping of all accrual
        if((reject.hyp[i, 1] || test.statistics[i, 2* (k-1) + 1] < futility.boundary[2* (k-1) + 1]) &&
           (reject.hyp[i, 2] || test.statistics[i, 2* (k-1) + 2] < futility.boundary[2* (k-1) + 2])   ){
           stage.decision[i, 1] = k
           stage.decision[i, 2] = k
           break;
        }
      }
      # If not already stopped for efficacy or futility for futility at stage K
      stage.decision[i, which(is.na(stage.decision[i, ]))] = K
      # Alpha re-allocation to stage K if one null hyp. rejected
      if(stage.decision[i, 1] == K && reject.hyp[i, 1] == 1 && reject.hyp[i, 2] == 0 && test.statistics[i, 2* (K-1) + 2] > est.eff.bound.alpha[2]){
        reject.hyp[i, 2] = 1
      }
      if(stage.decision[i, 1] == K && reject.hyp[i, 2] == 1 && reject.hyp[i, 1] == 0 && test.statistics[i, 2* (K-1) + 1] > est.eff.bound.alpha[1]){
        reject.hyp[i, 1] = 1
      }
    } # end for i loop
  ...
  } # end if(restrict.enrollment == FALSE)
}
